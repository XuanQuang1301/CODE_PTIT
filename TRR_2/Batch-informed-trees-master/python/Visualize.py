...
def draw_target(ax, center, color="red", label=None):
    y, x = center
    r = 5  # bán kính

    if color == "start_dual":  # custom: 2 nửa xanh - đỏ
        # Nửa trên: xanh
        wedge_top = Wedge(center=(x, y), r=r, theta1=0, theta2=180, facecolor="dodgerblue", alpha=0.9)
        ax.add_patch(wedge_top)

        # Nửa dưới: đỏ
        wedge_bottom = Wedge(center=(x, y), r=r, theta1=180, theta2=360, facecolor="red", alpha=0.9)
        ax.add_patch(wedge_bottom)

    else:
        # Goal hoặc các điểm bình thường: 1 màu
        circ = Circle((x, y), radius=r, color=color, alpha=0.9)
        ax.add_patch(circ)

    if label:
        ax.text(x, y + 6, label, fontsize=10, color="maroon", ha="center")
...
class Visualizer:
    ...
    def __init__(
        ...
        self.fig, self.ax = plt.subplots(figsize=(20, 20))
        self.fig.patch.set_facecolor("teal")      # canvas background
    ...
    def draw_tree(self, sim: int) -> None:
        ...
        for i in range(len(self.all_new_edges[sim])):
            ...
            draw_target(self.ax, self.start, color="start_dual", label="Start")


            draw_target(self.ax, self.goal, color="red", label="Goal")
    ...
    def redraw_map(self, sim: int) -> None:
        ...
        im = self.ax.imshow(self.occ_map, cmap="gray_r", extent=[0, 100, 100, 0])
